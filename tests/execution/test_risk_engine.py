...
def test_risk_engine_kelly_sizing(mock_config):
    ...
    fraction = engine.calculate_kelly_size(win_rate=0.55, win_loss_ratio=1.2, confidence=0.8)
...
def test_risk_engine_kill_switch(mock_config):
    ...
    # Even with an incredibly high AI conviction and high win rate
    fraction = engine.calculate_kelly_size(win_rate=0.80, win_loss_ratio=2.0, confidence=0.99)
...
def test_risk_engine_leverage_cap(mock_config):
    ...
    approved = engine.approve_order(proposed_side="BUY", proposed_fraction=4.0, current_exposure=0.0)
    
    assert approved == 3.0 # Capped at max_leverage
    
    # Attempt to add to a position that is already maxed
    approved_additional = engine.approve_order(side="BUY", requested_fraction=1.0, current_exposure=3.0)
