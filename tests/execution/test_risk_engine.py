...
def test_risk_engine_kelly_sizing(mock_config):
    ...
    fraction = engine.calculate_kelly_size(win_rate=0.55, win_loss_ratio=1.2, ai_conviction=0.8)
...
def test_risk_engine_kill_switch(mock_config):
    ...
    fraction = engine.calculate_kelly_size(win_rate=0.80, win_loss_ratio=2.0, ai_conviction=0.99)
...
def test_risk_engine_leverage_cap(mock_config):
    ...
    approved = engine.approve_order(side="BUY", requested_fraction=4.0, current_exposure=0.0)
