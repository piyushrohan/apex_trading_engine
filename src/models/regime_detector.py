...
class RegimeDetector:
    ...
    def detect(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        conditions = [
            (df["volatility_zscore"] < -1.0)
            & (abs(df["trend_slope"]) < 0.001),  # Chop/Compression
            (df["volatility_zscore"] > 1.5),  # Volatility Expansion
            (df["trend_slope"] > 0.002)
            & (df["volatility_zscore"] > 0.0),  # Strong Trend Up
            (df["trend_slope"] < -0.002)
            & (df["volatility_zscore"] > 0.0),  # Strong Trend Down
        ]
        ...
        df["regime_id"] = df["regime_str"].apply(
            lambda x: self.REGIMES.index(x) if x in self.REGIMES else 4
        )
