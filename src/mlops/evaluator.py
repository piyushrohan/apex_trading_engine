...
class ModelEvaluator:
    ...
    def evaluate_oos(self, pnl_series: pd.Series, trade_history: List[dict]) -> Dict[str, Any]:
        """
        ...
        if len(trade_history) < self.min_trades:
            logger.warning(f"Not enough trades for robust evaluation: {len(trade_history)}")
            metrics["sharpe"] = 0.0
        ...
        # Safety Gate Logic
        passed = (
            metrics["sharpe"] >= self.min_sharpe and
        ...
        if passed:
            logger.info(f"Model PASSED evaluation: Sharpe {sharpe:.2f}, MaxDD {max_dd:.2%}")
        else:
            logger.warning(f"Model FAILED evaluation: Sharpe {sharpe:.2f}, MaxDD {max_dd:.2%}")
