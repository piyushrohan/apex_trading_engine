...
class AutoRetrainPipeline:
    ...
    def execute_nightly_retrain(self):
        ...
        mock_trades = [{"pnl": 10} for _ in range(35)] + [
            {"pnl": -5} for _ in range(16)
        ]  # 51 trades

        metrics = self.evaluator.evaluate_oos(mock_pnl, mock_trades)

        _ = self.registry.register_model(new_model_id, "PPO", metrics)
        # agent.save(model_path)

        if metrics.get("passed_safety", False):
            logger.info(
                f"Candidate {new_model_id} passed safety gates. Promoting to SHADOW."
            )
