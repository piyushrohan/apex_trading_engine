...
class FeatureEngine:
    ...
    def add_orderflow_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['is_buy_liquidity_sweep'] = (lower_wick > (body * 2)) & (df['volume'] > df['volume'].rolling(20).mean())
        df['is_sell_liquidity_sweep'] = (upper_wick > (body * 2)) & (df['volume'] > df['volume'].rolling(20).mean())
        
        return df

    def add_relative_strength(self, target_df: pd.DataFrame, macro_df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculates ETH relative strength against BTC.
        Requires synchronized dataframes on the timestamp.
        """
        # Merge on timestamp
        merged = pd.merge(target_df, macro_df[['timestamp', 'close']], 
                          on='timestamp', how='left', suffixes=('', '_btc'))
    ...
    def process_all_features(self, target_df: pd.DataFrame, macro_df: pd.DataFrame) -> pd.DataFrame:
