...
class LiveTradePipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not prod_model_path:
            logger.warning(
                "No PROD model found in registry! System will run in monitoring mode."
            )

        self.meta_controller = MetaController(
            config
        )  # In reality, load weights from prod_model_path
    ...
    def _on_manual_position_change(self, symbol: str, position_data: dict):
        ...
        logger.info(
            f"Position sync detected change in {symbol}. "
            "Risk Engine adjusting exposure."
        )
    ...
    async def _trading_loop(self):
        ...
        # In production, connect to Binance_WS and iterate over ticks.
        ...
        while self._running:
            try:
                ...
                if action != 1:  # If not Flat
                    ...
                    approved_fraction = self.risk_engine.approve_order(
                        side, kelly_fraction, current_exposure=0.0
                    )

                    if approved_fraction > 0:
                        quantity = (
                            1.0  # Calculate based on equity * approved_fraction / price
                        )

                        # Execute Maker-Only Post-Only order
                        # We bid at BBO to capture the spread and pay 0 fees
                        price = (
                            current_bbo - 0.01 if side == "BUY" else current_bbo + 0.01
                        )

                        logger.info(
                            f"Executing {side} based on AI decision. "
                            f"Reason: {explanation['primary_reasons']}"
                        )
