...
class BacktestEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.initial_capital = config.get("environment", {}).get(
            "initial_capital", 1000.0
        )
        self.transaction_fee = config.get("environment", {}).get(
    ...
    def run(self, df_features: pd.DataFrame) -> Tuple[pd.Series, List[dict]]:
        ...
        Assumes close, regime_str, and 10 state vector columns exist.
        ...
        feature_cols = [
            ...
            if col not in ["timestamp", "close", "regime_str", "regime_id"]
        ]
        ...
        for idx, row in df_features.iterrows():
            ...
            action, conviction, _ = self.meta_controller.get_action(
                state_vector, regime
            )
        ...
        logger.info(
            f"Backtest complete. Final Equity: {equity:.2f}, "
