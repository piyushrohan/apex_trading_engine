...
class SecurityManager:
    ...
    def __init__(self, key_path: str = "configs/.keys.enc"):
        self.key_path = Path(key_path)
        self.master_key = os.getenv("APEX_MASTER_KEY")
        if not self.master_key:
            # During first run or setup, we might need to generate a master key
            logger.warning("APEX_MASTER_KEY environment variable not found. Encryption/Decryption will fail unless generating a new key.")
        else:
            try:
                self.cipher = Fernet(self.master_key.encode())
            except Exception as e:
                logger.error(f"Failed to initialize cipher with provided APEX_MASTER_KEY: {e}")
                self.cipher = None
                
    @staticmethod
    def generate_master_key() -> str:
        """Generates a new master key. Store this securely in your environment variables."""
        return Fernet.generate_key().decode()
        
    def encrypt_and_save_keys(self, api_key: str, api_secret: str):
    ...
    def load_and_decrypt_keys(self) -> dict:
        ...
        if not self.key_path.exists():
            raise FileNotFoundError(f"Encrypted key file not found at {self.key_path}. Run setup first.")
